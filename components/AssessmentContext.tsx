...
const STORAGE_KEY = 'firstmap_assessment';
...
function reducer(state: Assessment, action: AssessmentAction): Assessment {
  switch (action.type) {
    ...
    case 'TOGGLE_WORKER_ACTIVITY': {
      ...
      return {
        ...state,
        selectedWorkerActivities: exists
          ? state.selectedWorkerActivities.filter(a => a !== action.activity)
          : [...state.selectedWorkerActivities, action.activity],
      };
    }
    case 'TOGGLE_PARENT_ACTIVITY': {
      ...
      return {
        ...state,
        selectedParentActivities: exists
          ? state.selectedParentActivities.filter(a => a !== action.activity)
          : [...state.selectedParentActivities, action.activity],
      };
    }
...
export function AssessmentProvider({ children }: { children: React.ReactNode }) {
  const [state, dispatch] = useReducer(reducer, initialState, (init) => {
    ...
    try {
      ...
      if (stored) return JSON.parse(stored) as Assessment;
    } catch {}
    return init;
  });
